    ...
    if (request.body.email && request.body.password) {
      ...
      if (result) {
        if(result.status === 'verified'){
          ...
          let payload = {_id:result._id,username:result.username,email:result.email, role:result.role};
        ...
        return response.status(400).json({message:"Please verify your id through mail"});
      }
    }
