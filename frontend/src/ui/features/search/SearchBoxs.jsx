...
const SearchBoxs = () => {
  ...
  const setSearch = (event) => {
    ...
    const suggest = async () => {
      try {
        let response;
        const apiUrl = `https://flip-kart-clone-9xew.vercel.app//search?suggestion=${
  ...
  const handleSearch =(searchValue) =>{
    if(searchValue){
      ...
      dispatch({type:FILTER_SUCCESS,payload:{minValue:0,maxValue:10001}})
       dispatch(SearchProduct(`https://flip-kart-clone-9xew.vercel.app//search?search=${searchValue}`));
       setSearchValue('')
    }
  }
