...
const ProductsView = ({type}) => {
  ...
  if(type === 'search'){
    ...
    apiUrl = `http://localhost:8000/search?search=${searchParam}`
  }else if(type ==='category'){
    let categoryParam = queryParams.category;
    apiUrl = `http://localhost:8000/product_by_category?category=${categoryParam}`;
  }
  useEffect(()=>{
    
  dispatch(SearchProduct(apiUrl,queryParams.category))
  
  },[apiUrl])
  useEffect(()=> {
    setSlicedProducts(products.search_results.slice(0,10))
    ...
    const handlePageChange = (event, newPage) => {
      ...
      setSlicedProducts(products.search_results.slice((newPage - 1) * 10 , newPage*10 ))
  ...
  return (
    <>
    {
      products.loading === `${queryParams.category}` || products.loading === 'search' ?
    <Box style={{ backgroundColor: "white", width: "100%",minWidth:'520px', height: "100%",marginLeft:'10px',display:'flex',flexDirection:'column',alignItems:'space-around' }}>
      <SortProducts type = {type}/>
      ...
      <Box style={{ marginTop: 10,alignSelf:'center' ,display:'flex',justifyContent:'center',marginBottom:10}}>
        <Stack spacing={2}>
          <Pagination
            count={products.search_results.length === 0 ? 1 : Math.ceil(products.search_results.length / 10)}
            variant="outlined"
            shape="rounded"
