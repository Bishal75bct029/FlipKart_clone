...
const AddToCart = async (request, response) => {
  ...
  try {
    ...
    if (existingCartItem) {
      existingCartItem = {
        quantity: existingCartItem.quantity + request.body.quantity,
      };
  ...
  if (request.user.role === "buyer") {
    try {
      const cartItems = await CartSchema.find({ createdBy: request.user._id })
        .populate({
          path: "productID",
          select: "_id title price image discount tagline quantity,image",
        })
        .exec();
      return response
        .status(200)
        .json({ message: "Fetched successfully", cartItems });
    } catch (error) {
...
const deleteCart = async (request, response) => {
  if (request.params.id) {
    try {
      ...
      return response
        .status(200)
        .json({ message: "Deleted cart successfully" });
    } catch (error) {
